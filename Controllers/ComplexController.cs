  ...
  ) : ControllerBase
  {
    ...
    public async Task<ActionResult<Complex>> getComplex()
    {
      try
      {
        ...
        return Ok(ResponseMessage<Complex>.SuccessResult(complex));
      }
      catch (Exception ex)
      {
    ...
    )
    {
      ...
      {
        var response = ResponseMessage<Court>.Error($"Error creating court {ex.Message} ");
        return StatusCode(500, response);
      }
    }
    ...
    public async Task<ActionResult<Court>> UpdateCourt(int id, [FromBody] UpdateCourtDto updateDto)
    {
      ...
      {
        var response = ResponseMessage<Court>.Error($"Ocurrio un error al modificar la cancha: {ex.Message} ");
        return StatusCode(500, response);
      }
    }
